...
const Prescribtion = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:8082/getPrescriptionId')
      .then(response => {
  ...
  const handleSubmitPrescription = (e) => {
    ...
    try {
      axios.post('http://localhost:8082/create_prescription', formData);
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:8082/prescription_data')
  ...
  const DeleteMedicine = (Id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        // main code
        axios.delete("http://localhost:8082/delete_prescription/" + Id)
          .then(res => {
            console.log(res)
          })
        window.location.reload();
  ...
  // delete all data from medicine database column
  const deleteMedicineData = async () => {
    try {
      const response = await fetch('http://localhost:8082/deleteAllDataPrescription', {
        method: 'DELETE',
        headers: {
  ...
  // used for medicine edit drawer box
  ...
  // this part used medicine card image download
  const onDownload = () => {
    fetch()
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await axios.get('http://localhost:8082/latestDataPrescription');
  ...
  return (
    <div className="mt-3 grid h-full grid-cols-1 gap-5 xl:grid-cols-2 2xl:grid-cols-3">
      ...
      <div className="col-span-1 h-fit w-full xl:col-span-3 2xl:col-span-3">
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
              ...
              <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                  <thead class="text-xs text-white uppercase dark:bg-gray-700 dark:text-black" style={{backgroundColor:"#0C83F9"}}>
                    <tr>
                        ...
                        <tr class="border-b dark:border-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700">
                          ...
                          <td class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <div class="flex items-center space-x-4">
                              ...
                              <button onClick={() => DeleteMedicine(row.Id)} type="button" data-modal-target="delete-modal" data-modal-toggle="delete-modal" class="flex items-center text-red-700 hover:text-white border border-red-700 bg-red-300 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-3 py-2 text-center dark:border-red-500 dark:text-red-500 dark:hover:text-white dark:hover:bg-red-600 dark:focus:ring-red-900">
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-2 -ml-0.5" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true"
                                >
        ...
        <Modal title="Add Medicine" width={750} open={isModalOpen} onOk={handleOk} onCancel={handleCancel}>
          <hr className="my-5"></hr>
          <form onSubmit={handleSubmitPrescription}>
            <div class="relative z-0 w-full group flex justify-between">
              <label for="floating_email" class="text-black text-md font-semibold flex items-center">Prescription Id:  <p className='text-lg font-bold text-green-400'>" {maxId+1} "</p></label>
            ...
            <div class="grid md:grid-cols-2 md:gap-6 mt-7">
              <div class="relative z-0 w-full mb-3 group">
                <label for="birthday" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enter your medical name</label>
                <input type="text" id="birthday" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="medical name" required
                  value={medical_name} onChange={(e) => setMedical_name(e.target.value)}></input>
              </div>
              <div class="relative z-0 w-full mb-3 group">
                <label for="doctor" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Enter your doctor name</label>
                <input type="text" id="doctor" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light" placeholder="doctor name" required
                  value={doctor_name} onChange={(e) => setDoctor_name(e.target.value)}></input>
              </div>
            </div>

            <div>
              <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Doctor Notes</label>
              <textarea id="message" rows="4" class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Write your thoughts here..."
                value={doctor_notes} onChange={(e) => setDoctornotes(e.target.value)}></textarea>
            </div>
            ...
            <button type="submit" class="text-white mt-6 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
            <Link class="text-white ml-3 mt-6 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" to={'/prescription_list'}>Display</Link>
          </form>
        </Modal>
          ...
          {latestData.map(function fn(row) {
            return (
              <NftCardPrescription
                ...
                title={row.medical_name}
                author={row.doctor_name}
                price={row.doctor_notes}
                image={`http://localhost:8082/${row.imagePath}`}
              />
